...
ESP32 отправляет по I2S стерео‑поток (левый и правый каналы в одном DIN). Чтобы получить стерео на двух китайских модулях MAX98357A, их подключают параллельно по I2S, а выбор канала делают уровнем на входе `SD`.

### Общие соединения

Для обоих модулей MAX98357A одинаково:
...
Выходы каждого модуля (`SPK+/OUT+` и `SPK-/OUT-`) идут на свой динамик (левый и правый).

### Выбор левого и правого канала на модулях с SD(L+R)

На распространённых китайских платах SD часто уже подтянут к питанию через внутренний резистор порядка 1 МОм и подписан как `SD (L+R)`. В таком случае модуль по умолчанию даёт моно (сумму L+R).

Чтобы разделить каналы, на каждый модуль ставят дополнительный резистор между SD и землёй, образуя делитель напряжения вместе с тем самым внутренним 1 МОм. При разных напряжениях на SD чип переходит в режим левого или правого канала (подробные пороги смотри в даташите на MAX98357A).

Примерно так:

- для одного модуля (левый канал) подбирают такой резистор к GND, чтобы на SD было ближе к верхнему порогу (около 2 В при питании 5 В);
- для второго модуля (правый канал) — резистор большего номинала, чтобы напряжение на SD было ниже (около 1 В).

Точные значения зависят от конкретной платы и питания, но идея одна: оба модуля висят на одном I2S, а канал задаётся уровнем на SD. Код `BT_transiver.ino` при этом менять не нужно: I2S уже стерео, а PCM‑данные просто передаются в `i2s_write`.
